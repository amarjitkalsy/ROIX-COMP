...
import zstandard as zstd  # Using zstd for compression
...
def save_quantized_contours(contours_data, filename):
    """
    Save quantized contour data into a compressed binary file using zstd.
    ...
    with open(filename, 'wb') as f:
        ...
        with cctx.stream_writer(f) as compressor:
            pickle.dump(contours_data, compressor)
    compression_time = time.time() - start_time
    return compression_time

def load_quantized_contours(filename):
    """
    Load quantized contour data from a compressed binary file using zstd.
    ...
    with open(filename, 'rb') as f:
        ...
        with dctx.stream_reader(f) as decompressor:
            contours_data = pickle.load(decompressor)
...
def process_image(image_path, mode, value):
    ...
    # Return the per-image data and the size of the original image
    ...
    return per_image_data, size_original

def reconstruct_images(contours_data_list):
    """
    Reconstruct images from the quantized contours data list.
    contours_data_list is a list of per-image data dictionaries:
    {
        'image_name': image_name,
        'image_shape': image_shape,
        'contours_data': quantized_contours_data
    ...
    """
    for image_data in contours_data_list:
        ...
        # For each contour data, set pixel values
        for data in contours_data:
            row_index = data['row_index']
            x_start = data['x_start']
            x_end = data['x_end']
            quantized_values = np.array(data['quantized_values'], dtype=np.uint8)

            # Set the pixel values in the reconstructed image
            reconstructed_image[row_index, x_start:x_end+1] = quantized_values

        # Save the reconstructed image
        reconstructed_image_path = f'reconstructed_{image_name}'
        cv2.imwrite(reconstructed_image_path, reconstructed_image)
        print(f"Reconstructed image saved to {reconstructed_image_path}")

def main():
    # Paths
    image_folder = '/home/amarjitsingh/imgrec/xct/p1/sinograms' 
    ...
    value = 15    # Adjust the value as needed
    ...
    for per_image_data, size_original in results:
        if per_image_data:  # In case image failed to process
            all_quantized_contours_data.append(per_image_data)
            total_original_size += size_original

    # Save quantized contour data into a compressed binary file using zstd
    filename_contour_bin = 'contour_quantized.bin.zst'
    ...
    loaded_data, decompression_time = load_quantized_contours(filename_contour_bin)

    # Reconstruct images from the loaded data (if needed for quantized data)
    # reconstruct_images(loaded_data)
    ...
    for per_image_data_no_quant, size_original in results_no_quant:
        if per_image_data_no_quant:
            all_quantized_contours_data_no_quant.append(per_image_data_no_quant)
            total_original_size_no_quant += size_original

    # Save non-quantized contour data into a compressed binary file using zstd
    filename_contour_bin_no_quant = 'contour_nonquantized.bin.zst'
    ...
    # Load the data to measure decompression time
    loaded_data_no_quant, decompression_time_no_quant = load_quantized_contours(filename_contour_bin_no_quant)

    # Reconstruct images from the loaded data
    reconstruct_images(loaded_data_no_quant)
