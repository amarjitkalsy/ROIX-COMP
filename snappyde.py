...
logging.basicConfig(filename='snappy_de.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def decompress_snappy_file(file_info):
    input_file_path, output_directory = file_info
    with open(input_file_path, 'rb') as compressed_file:
        compressed_data = compressed_file.read()
    decompressed_data = snappy.decompress(compressed_data)
    output_file_name = os.path.basename(input_file_path).replace('.snappy', '.tif')
...
def main():
    ...
    compressed_files = [(os.path.join(compressed_directory, f), output_directory_decompressed) for f in os.listdir(compressed_directory) if f.endswith('.snappy')]
    with Pool(cpu_count()) as pool:
        pool.map(decompress_snappy_file, compressed_files)
    ...
    # Generate sinograms (if needed for other analysis)
    ...
    # Calculate absolute difference using decompressed images
